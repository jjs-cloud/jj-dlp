...
CONFIG_KEYS: tuple[_KeyDef, ...] = (
    ...
    _KeyDef("RESTART_ON_COLLAPSE",   "site",   "true", True, type="bool", comment="If the recording file's size ever drops between stall checks (e.g. yt-dlp truncates/reopens the output file from byte 0 after a live-stream reconnect), immediately restart the recording instead of silently treating the smaller size as the new baseline. (Default: true)"),
