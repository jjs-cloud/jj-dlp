...
CONFIG_KEYS: tuple[_KeyDef, ...] = (
    ...
    _KeyDef("NOTIFY_NO_CONFIRM_FILE", "global", "false", True,  "If the recording file cannot be confirmed (does not exist or no file growth) within one STALL_TIMEOUT, send a warning notification that the file could not be confirmed (true/false)."),
...
class IntroDelaySettingsPopup:
    ...
    def draw(self, stdscr) -> None:
        ...
        db.safe_addstr(stdscr, row, bx1 + 2, "Effective: ", curses.color_pair(db.C_NORMAL))
        db.safe_addstr(stdscr, row, bx1 + 13, effective_str[:box_w - 15], curses.color_pair(db.C_WARN) | curses.A_BOLD)
        row += 1
