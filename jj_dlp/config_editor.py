...
DOWNLOADER_FLAG_KEYS: tuple[_DlpFlagDef, ...] = (
    _DlpFlagDef("COOKIES_FROM_BROWSER", "--cookies-from-browser", "true",  preserve=False, type="bool", comment="Use cookies from the browser set in BROWSER (General section) when yt-dlp runs for this section. Set independently per [Checker]/[Downloader]/[LQ_Downloader]. Set to false to disable browser cookies for this section entirely."),
    _DlpFlagDef("DUMP_JSON",            "--dump-json",            "false", preserve=False, type="bool", comment="Have yt-dlp dump the stream's metadata as JSON instead of downloading. Typically used in [Checker] to detect whether a streamer is live."),
    _DlpFlagDef("NO_PART",              "--no-part",              "false", preserve=False, type="bool", comment="Do not use .part files while downloading; write directly to the final filename."),
    _DlpFlagDef("VERBOSE",              "--verbose",              "false", preserve=False, type="bool", comment="Print verbose debugging information from yt-dlp."),
    _DlpFlagDef("FIXUP",                "--fixup",                "",      preserve=False, type="str",  comment='How yt-dlp should fix up damaged output files after download (e.g. "never" to skip the fixup pass entirely).'),
    _DlpFlagDef("RETRIES",              "--retries",              "",      preserve=False, type="int",  comment="Number of times yt-dlp retries on a download error."),
    _DlpFlagDef("FORMAT",               "-f",                     "",      preserve=False, type="str",  comment="yt-dlp format selector (e.g. 4, 720p60, best). Typically used in [LQ_Downloader] to force a lower-quality fallback format."),
    _DlpFlagDef("DOWNLOADER_ARGS",      "--downloader-args",      "",      preserve=False, type="str",  comment='Extra arguments passed straight to the external downloader, e.g. ffmpeg:"-fps_mode passthrough -copyts -avoid_negative_ts make_zero".'),
    _DlpFlagDef("EXTRA_ARGS",           "",                       "",      preserve=False, type="str",  comment="Raw passthrough for any yt-dlp flag that doesn't have a dedicated key above (e.g. --some-flag value). Split the same way a shell command line would be."),
)
