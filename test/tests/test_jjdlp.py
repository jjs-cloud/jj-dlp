...
# ══════════════════════════════════════════════════════════════════════════════
# Debug Log Filter Tests
# ══════════════════════════════════════════════════════════════════════════════

@unittest.skipUnless(_jjdlp_importable(), "jj_dlp package not importable")
class TestDebugLogFilters(unittest.TestCase):
    """
    Regression tests for the tag-based filter gate in logger.dbg() and its
    dashboard callback (_dashboard_dbg_ref).

    Reported symptom: tags that should be suppressed (e.g. PERF, DISK, TWITCH)
    still appear on the Log tab, reproducible on Linux.

    Every test that touches the dashboard path verifies that the callback is
    (or is not) invoked — not just that the file-log is silent — because those
    are two independent code paths that must both honour the filter.
    """

    # ── setUp / tearDown ──────────────────────────────────────────────────────

    def setUp(self):
        from jj_dlp import logger as _log
        self.log = _log

        # Snapshot every filter value so tearDown can restore state exactly,
        # even if a test crashes mid-way.
        with _log._dbg_filters_lock:
            self._saved_filters = dict(_log.DBG_FILTERS)

        # Each test gets its own fresh capture list.
        self.captured: list = []

        # Inject a capturing dashboard callback.  Also supply a dummy
        # output-mode function so configure() doesn't blow up.
        _log.configure(lambda: 2, None, self.captured.append)

        # Keep file logging off — tests must not write to disk.
        _log.configure_debug_log(enabled=False, path="")

    def tearDown(self):
        # Restore all filter values atomically.
        with self.log._dbg_filters_lock:
            self.log.DBG_FILTERS.update(self._saved_filters)

        # Detach capture callback so it cannot bleed into subsequent tests.
        # configure() guards on `is not None`, so we write directly.
        self.log._dashboard_dbg_ref = None

    # ── shared helpers ────────────────────────────────────────────────────────

    def _has_tag(self, tag: str) -> bool:
        """Return True if any captured line contains the literal '[TAG]' string."""
        needle = f"[{tag}]"
        return any(needle in line for line in self.captured)

    # ── default filter states ─────────────────────────────────────────────────

    def test_default_false_tags_are_blocked(self):
        """
        Tags that ship as False (PERF, DISK, TWITCH, STALL) must never reach
        the dashboard callback without an explicit set_dbg_filter call.

        This is the primary regression guard for the reported bug.
        """
        default_off = ["PERF", "DISK", "TWITCH", "STALL"]
        for tag in default_off:
            self.captured.clear()
            self.log.dbg(f"[{tag}] must be suppressed by default")
            self.assertFalse(
                self._has_tag(tag),
                f"[{tag}] is False by default — must NOT reach the dashboard callback",
            )

    def test_default_true_tags_reach_dashboard(self):
        """
        Tags that ship as True (DRAIN, CHECKER, KILL, SPLIT, POPEN, UPDATER,
        CONFIG, POPUP) must reach the dashboard callback without any
        set_dbg_filter call.
        """
        default_on = ["DRAIN", "CHECKER", "KILL", "SPLIT", "POPEN",
                      "UPDATER", "CONFIG", "POPUP"]
        for tag in default_on:
            self.captured.clear()
            self.log.dbg(f"[{tag}] should pass through")
            self.assertTrue(
                self._has_tag(tag),
                f"[{tag}] is True by default — must reach the dashboard callback",
            )

    # ── set_dbg_filter() ──────────────────────────────────────────────────────

    def test_set_dbg_filter_false_blocks_enabled_tag(self):
        """set_dbg_filter(tag, False) must prevent the dashboard callback from firing."""
        self.log.set_dbg_filter("DRAIN", False)
        self.log.dbg("[DRAIN] should now be blocked")
        self.assertFalse(
            self._has_tag("DRAIN"),
            "After set_dbg_filter('DRAIN', False), [DRAIN] must not reach the dashboard",
        )

    def test_set_dbg_filter_true_unblocks_disabled_tag(self):
        """set_dbg_filter(tag, True) must allow messages to reach the dashboard."""
        self.log.set_dbg_filter("PERF", True)
        self.log.dbg("[PERF] should now be visible")
        self.assertTrue(
            self._has_tag("PERF"),
            "After set_dbg_filter('PERF', True), [PERF] must reach the dashboard",
        )

    def test_set_dbg_filter_toggle_true_then_false_leaves_tag_blocked(self):
        """True → False toggle must leave the tag blocked on the final state."""
        self.log.set_dbg_filter("PERF", True)
        self.log.set_dbg_filter("PERF", False)
        self.log.dbg("[PERF] second toggle should block this")
        self.assertFalse(
            self._has_tag("PERF"),
            "After True→False toggle [PERF] must still be blocked",
        )

    def test_set_dbg_filter_unknown_tag_is_silently_ignored(self):
        """set_dbg_filter() with a tag not in DBG_FILTERS must not add that tag."""
        before = set(self.log.DBG_FILTERS.keys())
        self.log.set_dbg_filter("PHANTOM_TAG_99", True)
        after = set(self.log.DBG_FILTERS.keys())
        self.assertEqual(before, after,
                         "set_dbg_filter must never inject unknown tags into DBG_FILTERS")

    # ── load_dbg_filters() ────────────────────────────────────────────────────

    def test_load_dbg_filters_enables_disabled_tag(self):
        """
        load_dbg_filters({tag: True}) must enable a tag that was False,
        so the next dbg() call reaches the dashboard.

        This mirrors the startup-restore path: if a user previously enabled
        PERF and that state is persisted to global.json, it must be honoured.
        """
        self.log.load_dbg_filters({"PERF": True})
        self.log.dbg("[PERF] should be visible after batch enable")
        self.assertTrue(
            self._has_tag("PERF"),
            "load_dbg_filters({'PERF': True}) must allow [PERF] to reach the dashboard",
        )

    def test_load_dbg_filters_disables_enabled_tag(self):
        """load_dbg_filters({tag: False}) must suppress a tag that was True."""
        self.log.load_dbg_filters({"DRAIN": False})
        self.log.dbg("[DRAIN] should be blocked after batch disable")
        self.assertFalse(
            self._has_tag("DRAIN"),
            "load_dbg_filters({'DRAIN': False}) must block [DRAIN] on the dashboard",
        )

    def test_load_dbg_filters_unknown_keys_not_injected(self):
        """load_dbg_filters() must silently skip keys absent from DBG_FILTERS."""
        before = set(self.log.DBG_FILTERS.keys())
        self.log.load_dbg_filters({"TOTALLY_FAKE": True, "ANOTHER_FAKE": False})
        after = set(self.log.DBG_FILTERS.keys())
        self.assertEqual(before, after,
                         "load_dbg_filters must not add unknown tags to DBG_FILTERS")

    def test_load_dbg_filters_does_not_alter_unspecified_tags(self):
        """load_dbg_filters() must only touch the keys it names; others are untouched."""
        snapshot = dict(self.log.DBG_FILTERS)
        self.log.load_dbg_filters({"PERF": True})   # only change PERF
        for tag, original_val in snapshot.items():
            if tag == "PERF":
                continue
            self.assertEqual(
                self.log.DBG_FILTERS[tag], original_val,
                f"load_dbg_filters must not alter unspecified tag [{tag}]",
            )

    # ── get_dbg_filters() snapshot isolation ──────────────────────────────────

    def test_get_dbg_filters_returns_independent_copy(self):
        """
        Mutating the dict returned by get_dbg_filters() must not affect the
        live DBG_FILTERS — the caller must receive a copy, not a view.
        """
        snapshot = self.log.get_dbg_filters()
        original_drain = snapshot["DRAIN"]
        snapshot["DRAIN"] = not original_drain          # mutate the copy

        # The live dict must still hold the original value.
        with self.log._dbg_filters_lock:
            live_val = self.log.DBG_FILTERS["DRAIN"]
        self.assertEqual(
            live_val, original_drain,
            "get_dbg_filters() must return an independent copy — not a live reference",
        )

    # ── message-format edge cases ─────────────────────────────────────────────

    def test_untagged_message_always_reaches_dashboard(self):
        """
        A message that does not start with '[' has no tag to filter on and
        must always be delivered to the dashboard callback.
        """
        self.log.dbg("plain message with no bracket prefix")
        self.assertTrue(
            len(self.captured) > 0,
            "Untagged messages must always reach the dashboard callback",
        )

    def test_unknown_tag_is_silently_dropped(self):
        """
        A message whose [TAG] is not present in DBG_FILTERS must be dropped.
        (DBG_FILTERS.get(tag, False) returns False for unknown tags.)
        """
        self.log.dbg("[TOTALLY_UNKNOWN_XYZ_999] should be dropped")
        self.assertFalse(
            self._has_tag("TOTALLY_UNKNOWN_XYZ_999"),
            "Messages with unrecognised tags must be silently dropped",
        )

    def test_compound_tag_filtered_by_outer_tag_when_enabled(self):
        """
        '[SPLIT][sub_context] ...' must be controlled by the outer [SPLIT] tag.
        When SPLIT is True the full compound message must reach the dashboard.
        """
        self.log.set_dbg_filter("SPLIT", True)
        self.log.dbg("[SPLIT][wait_for_streamer_file] looking for file")
        self.assertTrue(
            self._has_tag("SPLIT"),
            "Compound-tag message with enabled outer tag must reach the dashboard",
        )

    def test_compound_tag_blocked_when_outer_tag_disabled(self):
        """
        '[SPLIT][sub_context] ...' must be suppressed when SPLIT is False,
        regardless of the inner bracket group.
        """
        self.log.set_dbg_filter("SPLIT", False)
        self.log.dbg("[SPLIT][wait_for_streamer_file] must not appear")
        self.assertFalse(
            self._has_tag("SPLIT"),
            "Compound-tag message must be suppressed when outer tag is False",
        )

    def test_empty_bracket_message_bypasses_filter(self):
        """
        A message starting with '[]' has end==1, which fails the `end > 1`
        guard in dbg(), so the filter block is skipped entirely and the
        message passes through unconditionally.  This is the documented
        'no recognisable [TAG]' behaviour; the test pins it as intentional.
        """
        self.log.dbg("[] empty-tag prefix")
        self.assertTrue(
            len(self.captured) > 0,
            "Messages starting with [] should bypass tag filtering and reach the dashboard",
        )

    # ── dashboard callback mechanics ──────────────────────────────────────────

    def test_dashboard_callback_receives_timestamp_prefix(self):
        """Lines delivered to the dashboard must carry a [YYYY-MM-DD HH:MM:SS] prefix."""
        import re
        self.log.set_dbg_filter("DRAIN", True)
        self.log.dbg("[DRAIN] check timestamp format")
        self.assertTrue(self.captured, "No line was captured")
        self.assertRegex(
            self.captured[-1],
            r"\[\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\]",
            "Dashboard lines must begin with a [YYYY-MM-DD HH:MM:SS] timestamp",
        )

    def test_dashboard_callback_not_invoked_for_filtered_tag(self):
        """
        The dashboard callback must not be called at all when a tag is filtered
        out — not just 'the content should differ', but zero invocations.
        """
        call_count = []
        self.log._dashboard_dbg_ref = lambda msg: call_count.append(msg)
        self.log.set_dbg_filter("PERF", False)
        self.log.dbg("[PERF] filtered — callback must not fire")
        self.assertEqual(
            len(call_count), 0,
            "Dashboard callback must not be invoked at all for a filtered-out tag",
        )

    def test_site_name_appears_in_dashboard_line(self):
        """When site_name is passed to dbg(), it must appear as [site_name] in the line."""
        self.log.set_dbg_filter("DRAIN", True)
        self.log.dbg("[DRAIN] per-site message", site_name="mysite")
        self.assertTrue(
            any("[mysite]" in line for line in self.captured),
            "site_name must appear as [site_name] in the dashboard line",
        )

    def test_file_logging_disabled_does_not_suppress_dashboard(self):
        """
        The dashboard callback must fire even when file-logging is disabled.
        The Log tab toggle must work independently of the debug-file toggle.
        """
        self.log.configure_debug_log(enabled=False, path="")
        self.log.set_dbg_filter("DRAIN", True)
        self.log.dbg("[DRAIN] dashboard only, no file")
        self.assertTrue(
            self._has_tag("DRAIN"),
            "Dashboard callback must fire regardless of whether file-logging is on",
        )

    # ── thread safety (primary Linux regression) ───────────────────────────────

    def test_concurrent_filter_mutations_and_dbg_no_exceptions(self):
        """
        Rapid concurrent filter writes (set_dbg_filter) interleaved with
        concurrent dbg() calls must not raise any exceptions, lose any
        filter keys, or corrupt DBG_FILTERS.

        Linux schedules threads across multiple cores more aggressively than
        Windows, which makes this the most likely place for a race condition
        to surface.
        """
        errors = []
        ...
        def _toggle():
            ...
            while not stop.is_set():
                self.log.set_dbg_filter("DRAIN", bool(count % 2))
                count += 1

        def _emit():
            while not stop.is_set():
                try:
                    self.log.dbg("[DRAIN] concurrent emission")
                except Exception as exc:
                    errors.append(exc)

        workers = (
            [threading.Thread(target=_toggle, daemon=True) for _ in range(3)]
            + [threading.Thread(target=_emit,   daemon=True) for _ in range(3)]
        )
        for w in workers:
            w.start()

        time.sleep(0.3)   # enough time to expose scheduling races on Linux
        stop.set()
        for w in workers:
            w.join(timeout=2)

        self.assertEqual(errors, [],
                         f"dbg() raised exception(s) under concurrent access: {errors}")

        # The key set must be intact — no phantom keys, no missing keys.
        with self.log._dbg_filters_lock:
            live_keys = set(self.log.DBG_FILTERS.keys())
        self.assertEqual(
            live_keys,
            set(self._saved_filters.keys()),
            "Concurrent access must not corrupt the DBG_FILTERS key set",
        )

    def test_concurrent_load_dbg_filters_and_dbg_no_exceptions(self):
        """
        Concurrent load_dbg_filters() calls (the startup-restore path) must
        not race with dbg() emissions and must not raise exceptions.
        """
        errors = []
        stop = threading.Event()
        ...
        def _batch_update():
            while not stop.is_set():
                try:
                    self.log.load_dbg_filters({"PERF": True, "DRAIN": False})
                    self.log.load_dbg_filters({"PERF": False, "DRAIN": True})
                except Exception as exc:
                    errors.append(exc)
        ...
        def _emit():
            while not stop.is_set():
                try:
                    self.log.dbg("[DRAIN] load_dbg_filters race test")
                    self.log.dbg("[PERF] load_dbg_filters race test")
                except Exception as exc:
                    errors.append(exc)

        workers = (
            [threading.Thread(target=_batch_update, daemon=True) for _ in range(2)]
            + [threading.Thread(target=_emit,        daemon=True) for _ in range(2)]
        )
        for w in workers:
            w.start()
        ...
        self.assertEqual(errors, [],
                         f"Exception(s) during concurrent load_dbg_filters + dbg: {errors}")
